    ...
    **The hook has the lowest priority (level 10)**.
    ...
    def _write_console(self) -> None:
        ...
        if iter_time is not None:
            eta_seconds = iter_time * (self.args.max_iters - self.args.cur_iter - 1)
        ...
        process_string = "Epoch: [{}][{}/{}]".format(self.args.cur_epoch, self.args.inner_iter,
                                                     self.args.iters_one_epoch - 1)
    ...
    def _write_tensorboard(self) -> None:
        # Write only when the data is updated(iter > self._last_write) 
